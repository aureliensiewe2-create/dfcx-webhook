...
app.post("/df-webhook", (req, res) => {
  ...
  const lang =
    (req.body?.sessionInfo?.languageCode || "en").toLowerCase();
  ...
  if (tag === "track-order") {
    ...
    // 1) Demander le numéro si absent/incorrect
  ...
  // Réponse par défaut si tag inconnu
  return res.json({
    fulfillment_response: {
      messages: [{ text: { text: ["Sorry, I didn’t understand.", "Désolé, je n’ai pas compris."] } }],
    },
  });
});

// ----- Produits : recherche simple -----
app.post("/search-products", async (req, res) => {
  ...
  const products = [
    ...
    { name: "Jeans Slim", color: "blue", price: 45, category: "jean", size: "32", brand: "Levi's" },
  ];

  // Filtrage robuste
  const result = products.filter((p) => {
    const matchColor = !color || (p.color && p.color.toLowerCase() === String(color).toLowerCase());
    const matchCategory = !category || (p.category && p.category.toLowerCase() === String(category).toLowerCase());
    const matchBrand = !brand || (p.brand && p.brand.toLowerCase().includes(String(brand).toLowerCase()));
    const matchPrice = !price_max || (p.price && p.price <= Number(price_max));
    const matchSize = !size || (p.size && p.size.toLowerCase() === String(size).toLowerCase());

    return matchColor && matchCategory && matchBrand && matchPrice && matchSize;
  });
